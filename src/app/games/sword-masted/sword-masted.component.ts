...
import { BusService } from 'src/app/core/bus.service';
...
export class SwordMasterComponent implements OnInit {
  ...
  constructor(private wss: WebSocketService, private chatService: ChatService, private bus: BusService) { }
  ...
  ngOnInit(){
    this.chatService.getNewMessage().subscribe((message: any) => {
      if(message.success == 'wordUnHide'){
        this.bus.trigerNotify('слово угадано')
        this.messageList = []
      } else if(message.error == 'wordNotFound'){
        this.bus.trigerNotify('слово не найдено в словаре')
  ...
  sendMessage() {
    ...
    if(userWord.length == 0) return this.bus.trigerNotify('empty')
    if(userWord.length != 5){
        this.bus.trigerNotify('слово должно быть 5 букв')
        this.wordInput.nativeElement.value = ''

    } else if(wordsList.includes(userWord)){
        this.bus.trigerNotify('слово уже использовано')
        this.wordInput.nativeElement.value = ''
